...
final class KoefIntMatrix {
    ...
    init(im: [[Element]]) {
        ...
        for j in 0 ..< im[0].count {
            ...
            for row in im {
                let e = row[j]
                for (_, w) in e.contents {
                    if !colWays.contains(where: { $0.isEq(w) }) { colWays.append(w) }
                }
            }
            for w in colWays {
                transpItems.append(im.map { row in
                    let e = row[j]
                    if let idx = e.contents.firstIndex(where: { $0.1.isEq(w) }) {
                        return NumInt(n: e.contents[idx].0.n)
                    } else {
                        return NumInt(n: 0)
                    }
                })
