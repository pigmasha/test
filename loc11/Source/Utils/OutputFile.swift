...
final class OutputFile {
    ...
    static func setFileName(fileName: String, forceClear: Bool = false) throws {
        self.fileName = fileName
        if !fileNames.contains(fileName) || forceClear {
            let h = PathAlg.isTex ? headerTex : header
            try h.write(toFile: fileName, atomically: true, encoding: .utf8)
            fileNames += [ fileName ]
        }
    }
