...
enum ArrType: Int, Equatable, CaseIterable {
    ...
    var from: VertexType {
        switch self {
        case .a21, .a31: return .e1
        case .a12, .a32: return .e2
        case .a13, .a23: return .e3
        }
    }

    var to: VertexType {
        switch self {
        case .a12, .a13: return .e1
        case .a21, .a23: return .e2
        case .a31, .a32: return .e3
        }
    }
}
...
final class Way {
    ...
    var str: String {
        ...
        let isAlpha: Bool
        switch endArr {
        case .a21, .a32, .a13: isAlpha = false
        case .a12, .a23, .a31: isAlpha = true
        }
        if let t = twin, !isAlpha { return t.str }
        let alphaPrefix = PathAlg.isTex ? "\\a_{" : "&alpha;<sub>"
        let suffix = len == 2 || len == 3 ? (PathAlg.isTex ? "}" : "</sub>") : (PathAlg.isTex ? "}^{\(len / 2)}" : "</sub><sup>\(len / 2)</sup>")
        if len % 2 == 0 {
            let prefix = isAlpha ? alphaPrefix : (PathAlg.isTex ? "\\b_{" : "&beta;<sub>")
            return prefix + endArr.to.str + suffix
        } else {
            ...
            if len == 1 {
                ...
                return alphaPrefix + endArr.to.str + suffix + aPrefix + endArr.str + aSuffix
            } else {
                return aPrefix + endArr.str + aSuffix + alphaPrefix + endArr.from.str + suffix
            }
        }
    }
