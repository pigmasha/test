...
final class BimodQ {
    ...
    private static func evenQ(_ m: Int) -> (pij: [IntPair], sizes: [NumInt]) {
        ...
        sizes += [NumInt(intValue: f(m, 3)+1), NumInt(intValue: f(m, 3)+1)]
        ...
        sizes += [NumInt(intValue: f(m, 2)+1), NumInt(intValue: f(m, 2)+1)]

        for i in 0 ... f(m, 3) {
            p_ij += [IntPair(n0: 4*(m+1)-h(m,3)+i, n1: 3)]
        }
        sizes += [NumInt(intValue: f(m, 3)+1)]
