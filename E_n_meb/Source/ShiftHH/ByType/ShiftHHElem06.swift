...
final class ShiftHHElem06 : ShiftHHElem {
    ...
    override func shift0(_ hhElem: HHElem, degree: Int, shift: Int, n: Int, s: Int, m: Int, ell_0: Int, ell: Int) {
        ...
        for j in 3*s ..< 4*s {
            HHElem.addElemToHH(hhElem, i:j-s, j:j, leftFrom:4*j+1, leftTo:4*j+3, right:4*j+1, koef:-PathAlg.k1J(ell, j:j, m:m))
        }
        for j in 4*s ..< 5*s {
            HHElem.addElemToHH(hhElem, i:j-s, j:j, leftFrom:4*j+2, leftTo:4*(j+1), right:4*j+2, koef:-PathAlg.k1JPlus1(ell, j:j, m:m))
        }
        for j in 6*s ..< 7*s {
