//
//  Created by M on 24.05.2018.
//

import Foundation

final class ShiftHHElem18 : ShiftHHElem {
    init() {
        super.init(type:18)
    }
    ...
    override func shift0(_ hhElem: HHElem, degree: Int, shift: Int, n: Int, s: Int, m: Int, ell_0: Int, ell: Int) {
        ...
        for j in 5*s ..< 6*s {
            HHElem.addElemToHH(hhElem, i:j, j:j,
                               ...
                               rightFrom:4*j+3, rightTo:4*j+3, koef:-PathAlg.k1JPlus1(ell, j:j, m:m+4))
        }
    }
    ...
    override func shift1(_ hhElem: HHElem, degree: Int, shift: Int, n: Int, s: Int, m: Int, ell_0: Int, ell: Int) {
        ...
        for j in 0 ..< s {
            let k = PathAlg.k1J(ell, j:j+4, m:m)
            HHElem.addElemToHH(hhElem, i:j, j:j,
    ...
    override func shift3(_ hhElem: HHElem, degree: Int, shift: Int, n: Int, s: Int, m: Int, ell_0: Int, ell: Int) {
        ...
        for j in 3*s ..< 4*s {
            HHElem.addElemToHH(hhElem, i:myMod2S(j+1), j:j,
                               ...
                               rightFrom:4*(j+s)+2, rightTo:4*(j+1), koef:-f1(j, 4*s - 1))
        }
        for j in 4*s ..< 5*s {
            HHElem.addElemToHH(hhElem, i:myMod2S(j+1), j:j,
                               leftFrom:4*(j+m+1)+2, leftTo:4*(j+m+1)+2,
                               rightFrom:4*(j+s)+2, rightTo:4*(j+1), koef:f1(j, 5*s - 1))
        }
        for j in 5*s ..< 6*s {
            let k = -PathAlg.k1JPlus1(ell, j: j, m: m+4) * f1(j, 6*s - 1)
        ...
        for j in 4*s ..< 6*s {
            if j < 5*s - 1 || j == 6*s - 1 {
                HHElem.addElemToHH(hhElem, i:+myModS(j+1), j:j,
                                   ...
                                   rightFrom:4*j+2, rightTo:4*(j+1), koef:PathAlg.k1J(ell, j:j+4, m:m)*f1(j,6*s-1))
            }
        }
    }
    ...
    override func shift5(_ hhElem: HHElem, degree: Int, shift: Int, n: Int, s: Int, m: Int, ell_0: Int, ell: Int) {
        ...
        for j in 5*s ..< 6*s {
            HHElem.addElemToHH(hhElem, i:2*s+myModS(j+1), j:j,
                               ...
                               rightFrom:4*j+3, rightTo:4*(j+s+1+s*f(j,6*s-1))+1,
                               koef:-PathAlg.k1J(ell+1, j: j, m: m)*f1(j,6*s-1))
        }
    }
    ...
    override func shift6(_ hhElem: HHElem, degree: Int, shift: Int, n: Int, s: Int, m: Int, ell_0: Int, ell: Int) {
        ...
        for j in 2*s ..< 3*s {
            let k = PathAlg.k1J(ell, j: j, m: m+4)*f2(j%s,s-1)
            HHElem.addElemToHH(hhElem, i:j-s, j:j,
                               leftFrom:4*(j+m)+1, leftTo:4*(j+m)+3,
        ...
        /*let f: (Int) -> Int = { j in
            return PathAlg.k1J(ell+1, j: j, m: m+s-1)
            //if s == 7 { return (ell == 18 ? -1 : 1) * ([1, 5, 6].contains(j%s) ? -1 : 1) }
        }*/
        //printK(prefix: "s=\(s)", jFrom: 4*s, jTo: 5*s - 1, m: m, ell: ell, f: f)
        for j in 4*s ..< 6*s {
            if j < 5*s - 1 || j == 6*s - 1 {
                HHElem.addElemToHH(hhElem, i:+myModS(j+1), j:j,
        ...
        for j in 6*s ..< 7*s {
            HHElem.addElemToHH(hhElem, i:3*s+myModS(j+1)-2*s*f(j, 7*s-1), j:j,
        ...
        for j in 7*s ..< 8*s {
            HHElem.addElemToHH(hhElem, i:s+myModS(j+1)+2*s*f(j, 8*s-1), j:j,
                               leftFrom:4*(j+m+s+1)+1, leftTo:4*(j+m+s+1)+1,
                               rightFrom:4*j+3, rightTo:4*(j+s+1)+1, koef:-1)
        }
    }
    ...
    override func shift7(_ hhElem: HHElem, degree: Int, shift: Int, n: Int, s: Int, m: Int, ell_0: Int, ell: Int) {
        ...
        /*let f: (Int) -> Int = { j in
            if s == 5 { return [1, 2, 3].contains(j%s) ? -1 : 1 }
            return 1
        }
        printK(prefix: "s=\(s)", jFrom: s, jTo: 2*s, m: m, ell: ell, f: f)*/
        for j in s ..< 2*s {
    ...
    override func shift8(_ hhElem: HHElem, degree: Int, shift: Int, n: Int, s: Int, m: Int, ell_0: Int, ell: Int) {
        ...
        for j in 6*s ..< 8*s {
            HHElem.addElemToHH(hhElem, i:s+myMod2S(j+s+1), j:j,
                               ...
                               rightFrom:4*j+3, rightTo:4*(j+s+1)+1, koef:-f1(j, 7*s))
        }
    }
    ...
    override func shift9(_ hhElem: HHElem, degree: Int, shift: Int, n: Int, s: Int, m: Int, ell_0: Int, ell: Int) {
        ...
        for j in 7*s ..< 8*s {
            HHElem.addElemToHH(hhElem, i:+myModS(j+1), j:j,
                               leftFrom:4*(j+m+1)+3, leftTo:4*(j+m+1)+3,
                               rightFrom:4*j+3, rightTo:4*(j+1), koef:-PathAlg.k1J(ell+1, j: j, m: m-1)*f1(j,8*s-1))
        }
    }
    ...
    override func shift10(_ hhElem: HHElem, degree: Int, shift: Int, n: Int, s: Int, m: Int, ell_0: Int, ell: Int) {
        ...
        for j in 2*s ..< 3*s {
            ...
            HHElem.addElemToHH(hhElem, i:j+s, j:j,
                               ...
                               rightFrom:4*j+1, rightTo:4*j+2, koef:k)
            HHElem.addElemToHH(hhElem, i:j+3*s, j:j,
                               leftFrom:4*(j+m)+3, leftTo:4*(j+m)+3,
                               rightFrom:4*j+1, rightTo:4*j+3, koef:k)
        }
        for j in 3*s ..< 4*s {
            let k = PathAlg.k1J(ell+1, j: j, m: m-2)
            HHElem.addElemToHH(hhElem, i:j-s, j:j,
