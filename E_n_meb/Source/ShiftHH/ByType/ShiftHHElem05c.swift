...
final class ShiftHHElem05c : ShiftHHElem {
    ...
    override func oddShift4(_ hhElem: HHElem, degree: Int, shift: Int, n: Int, s: Int, m: Int, ell: Int) {
        ...
        j = s == 1 ? 2 : 4*s+myModS(1)
        let j1 = s
        HHElem.addElemToHH(hhElem, i:+myModS(j+1), j:j,
                           leftFrom:4*(j+m)+3, leftTo:4*(j+m+j1+1+s*f(s,1))+2,
                           rightFrom:4*(j+j1+s*f(s,1))+1, rightTo:4*(j+1),
        ...
        HHElem.addElemToHH(hhElem, i:3*s+myModS(j+1), j:j,
                           leftFrom:4*(j+m+j1+1+s*f(s,1))+1, leftTo:4*(j+m+1)+3,
                           rightFrom:4*j+3, rightTo:4*(j+s-j1+1+s*f(s,1))+1, koef:-f2(j%s, myModS(s-2))*PathAlg.kGamma(ell, j: j, m: m))
        HHElem.addElemToHH(hhElem, i:7*s+myModS(j+1), j:j,
                           leftFrom:4*(j+m+j1+1+s*f(s,1))+2, leftTo:4*(j+m+1)+3,
                           rightFrom:4*j+3, rightTo:4*(j+s-j1+1+s*f(s,1))+2, koef:f2(j%s, myModS(s-2))*PathAlg.kGamma(ell, j: j, m: m))
    }
