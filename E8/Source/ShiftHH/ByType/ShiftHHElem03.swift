...
final class ShiftHHElem03 : ShiftHHElem {
    ...
    override func shift1(_ hhElem: HHElem, s: Int, m: Int, ell: Int) {
        ...
        for j in 6*s ..< 7*s {
            HHElem.addElemToHH(hhElem, i:j-s, j:j, leftFrom:8*(j+m)+7, leftTo:8*(j+m+1)+1, rightFrom:8*j+4, rightTo:8*j+4, koef:-1)
        }
        for j in 7*s ..< 8*s {
            HHElem.addElemToHH(hhElem, i:j, j:j, leftFrom:8*(j+m)+7, leftTo:8*(j+m+1)+1, rightFrom:8*j+5, rightTo:8*j+6, koef:-1)
        }
        for j in 8*s ..< 9*s {
    ...
    override func shift28(_ hhElem: HHElem, s: Int, m: Int, ell: Int) {
        ...
        for j in 6*s ..< 7*s {
            HHElem.addElemToHH(hhElem, i:+myModS(j+1), j:j, leftFrom:8*(j+m+1), leftTo:8*(j+m+1)+1, rightFrom:8*j+6, rightTo:8*(j+1), koef:-1)
        }
        for j in 7*s ..< 8*s {
