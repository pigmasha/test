...
final class ShiftHHElem16c : ShiftHHElem {
    ...
    override func oddShift1(_ hhElem: HHElem, s: Int, m: Int, ell: Int) {
        ...
        j += 3*s
        HHElem.addElemToHH(hhElem, i:+myModS(j+1), j:j, leftFrom:8*(j+m+1)+1, leftTo:8*(j+m+2), rightFrom:8*j+5, rightTo:8*(j+1), koef:-1)
    }
    ...
    override func oddShift14(_ hhElem: HHElem, s: Int, m: Int, ell: Int) {
        ...
        let j = 0
        HHElem.addElemToHH(hhElem, i:+myModS(j+1), j:j, leftFrom:8*(j+m)+7, leftTo:8*(j+m+1)+1, rightFrom:8*j, rightTo:8*(j+1), koef:1, noZeroLenR:true)
