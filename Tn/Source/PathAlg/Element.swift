...
final class Element {
    ...
    init(way: Way, koef: Int = 1) {
        contents = []
        add(way: way, koef: koef)
    }

    init(way1: Way, way2: Way) {
        contents = []
        add(way: way1, koef: 1)
        add(way: way2, koef: 1)
    }
    ...
    var strKey: String {
        if contents.count != 1 { fatalError("strKey for bad elem=" + str) }
        return contents[0].1.str
    }
    ...
    func add(element: Element, koef: Int) {
        for (k, w) in element.contents {
            add(way: w, koef: k.n * koef)
        }
    }
