...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    private let nFields: FieldsTuple = (from: NSTextField(frame: .zero), to: NSTextField(frame: .zero), label: "n")
    private let charFields: FieldsTuple = (from: NSTextField(frame: .zero), to: NSTextField(frame: .zero), label: "char")
    ...
    private var nInterval: IntervalTuple = (from: 0, to: 0)
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        tupleFields = [nFields, charFields]
        ...
        y -= 44
        var x: CGFloat = 50
        addTuple(x, y, nFields)
        addTuple(x + 170, y, charFields)

        addButton(btClear, to: v, title: "Clear html", action: #selector(onClear), autoSz: .minYMargin, x: 280 + kButtonW, y: y)
    ...
    @objc func onRun() {
        ...
        if let nI = readInterval(nFields), let charI = readInterval(charFields) {
            nInterval = nI
            charInterval = charI
        } else { return }
        guard nInterval.from > 3 else { self.addInfoStr("ERROR! n.from < 4"); return }
        PathAlg.n = nInterval.from
