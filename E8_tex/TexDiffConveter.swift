...
struct TexDiffConveter {
        ...
        for d in 0 ..< 29 {
            ...
            for i in 1 ..< fors.count {
                ...
                while j < lines.count {
                    ...
                    throw TexDiffConverterError.badLine("for \(i), \(j): bad prefix, line=\(line); d=\(d)")
                }
    ...
    private static func koefStr(from part: String) -> String {
        ...
        default:
            var s = part
            s = s.replacingOccurrences(of: "minusDeg(j_1)", with: "(-1)^{j_1}")
            s = s.replacingOccurrences(of: "minusDeg(j_1+1)", with: "(-1)^{j_1+1}")
            return "(" + s + ")"
        }
    }
}
